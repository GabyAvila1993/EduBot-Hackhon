...
const MCPLanguageExercises: React.FC = () => {
  ...
  useEffect(() => {
    const fetchExercises = async () => {
      try {
        setLoading(true);
        const response = await apiService.getExercisesByModule('lengua');
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      case 'grammar': return 'text-blue-500 bg-blue-100 dark:bg-blue-900';
      case 'vocabulary': return 'text-purple-500 bg-purple-100 dark:bg-purple-900';
      case 'reading': return 'text-green-500 bg-green-100 dark:bg-green-900';
      case 'writing': return 'text-orange-500 bg-orange-100 dark:bg-orange-900';
      default: return 'text-red-500 bg-red-100 dark:bg-red-900';
    }
  };
  ...
  const getTypeLabel = (type: string) => {
    switch (type) {
      ...
      case 'reading': return 'Comprensión';
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">Cargando ejercicios de lengua...</p>
        </div>
      </div>
    );
  }
  ...
  if (exercises.length === 0) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <p className="text-gray-600 dark:text-gray-400">No se encontraron ejercicios de lengua y literatura.</p>
          <Link to="/cursos" className="text-red-500 hover:text-red-600 mt-4 inline-block">
            Volver a Cursos
          </Link>
        </div>
      </div>
      ...
      <div className="p-8 max-w-4xl mx-auto">
        <div className="text-center bg-white dark:bg-gray-800 rounded-xl p-8 shadow-lg">
          <div className="mb-6">
            ...
            <p className="text-gray-600 dark:text-gray-400">Has terminado todos los ejercicios de lengua y literatura.</p>
          </div>
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <div className="mb-6">
        <Link to="/cursos" className="inline-flex items-center text-red-500 hover:text-red-600 mb-4">
          <span className="material-symbols-outlined mr-2">arrow_back</span>
          Volver a Cursos
        </Link>
        
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Ejercicios de Lengua y Literatura</h1>
          <div className="flex gap-2">
            <span className={`px-3 py-1 rounded-full text-sm font-medium ${getTypeColor(currentExercise.type)}`}>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl p-8 shadow-lg">
        ...
        <div className="flex justify-between">
          <div className="flex items-center text-sm text-gray-500 dark:text-gray-400">
            <span className="material-symbols-outlined mr-1">translate</span>
            Lengua y Literatura - Nivel Primaria
          </div>
...
export default MCPLanguageExercises;
